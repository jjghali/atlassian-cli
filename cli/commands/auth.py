...
@click.group()
def auth():
    click.echo("This is the auth command")
...
def login():
    ...
    is_cloud = click.confirm('Are you using a Cloud instance? Cloud instances contains *.atlassian.net in the URL', abort=True)
    ...
    if not is_cloud:
        ...
        atlassian_url = click.prompt('Please enter the URL for your Atlassian product (companyname.atlassian.net):')
    
    username = click.prompt('Please enter the username:')
    password = click.prompt('Please enter the password:', hide_input=True)
    
    skipssl = click.confirm('Do you wish to skip SSL validation? It is recommended to not skip it for security reasons', default=False)
